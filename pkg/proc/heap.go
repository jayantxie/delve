...
func (s *HeapScope) readHeap(scope *EvalScope) {
	ptrSize := s.bi.Arch.PtrSize()
	exeimage := s.bi.Images[0]
	...
	if mheap.HasField("spans") {
		...
		arenaBaseOffset := s.getArenaBaseOffset(scope)
	...
	s.readSpans(scope, mheap, arenas)
}

func (s *HeapScope) getArenaBaseOffset(scope *EvalScope) int64 {
...
func (s *HeapScope) readSpans(scope *EvalScope, mheap *region, arenas []arena) {
	...
	for i := int64(0); i < n; i++ {
		...
		case spanInUse:
			...
			for special := sp.Field("specials"); special.Address() != 0; special = special.Field("next") {
				...
				if special.Field("kind").Uint8() != uint8(scope.rtConstant("_KindSpecialFinalizer")) {
